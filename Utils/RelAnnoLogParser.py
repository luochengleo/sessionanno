...
patterns = {key: re.compile('%s=(.*?)\\t' % key) for key in ['TIMESTAMP', 'ANNOTATOR', 'TASK', 'QUERY', 'ACTION']}
...
def fromString(line):
    annotatorID = patterns['ANNOTATOR'].search(line).group(1)
    ...
    anno_log_obj = RelAnnotation.objects.create(annotatorID=annotatorID,
                                task_id=task_id,
                                query=query,
                                result_id=result_id,
